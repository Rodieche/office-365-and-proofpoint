...
const scriptPath = './powershell/ExchangeConnection.ps1';

const command = `powershell.exe -ExecutionPolicy Bypass -File "${scriptPath}"`;

exec(command, { shell: 'runas' }, (error, stdout, stderr) => {
    if (error) {
        console.error(`Error executing PowerShell script as admin: ${error}`);
        return;
    }
